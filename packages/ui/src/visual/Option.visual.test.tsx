...
describe('Option visual parity with @mui/joy', () => {
  for (const scheme of COLOR_SCHEMES) {
    for (const variant of VARIANTS) {
      for (const color of COLORS) {
        it(`${variant}/${color} resting state matches Joy UI's computed styles in ${scheme}`, async () => {
          ...
          await expect(page.getByTestId(`joy-${variant}-${color}`)).toMatchScreenshot(`option-${variant}-${color}-joy-${scheme}`);
          await expect(page.getByTestId(`hintoric-${variant}-${color}`)).toMatchScreenshot(`option-${variant}-${color}-hintoric-${scheme}`);
        });
      }
    }
    ...
    it(`selected option gets the variant Active background (no font-weight change) in ${scheme}`, async () => {
      await setColorScheme(scheme);
