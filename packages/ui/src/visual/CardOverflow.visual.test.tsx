...
describe('CardOverflow visual parity with @mui/joy', () => {
  for (const scheme of COLOR_SCHEMES) {
    it(`bleeds past the parent Card padding, matching Joy UI in ${scheme}`, async () => {
      ...
      await expect(page.getByTestId('joy-card')).toMatchScreenshot(`cardoverflow-joy-${scheme}`);
      await expect(page.getByTestId('hintoric-card')).toMatchScreenshot(`cardoverflow-hintoric-${scheme}`);
    });
  }
});
