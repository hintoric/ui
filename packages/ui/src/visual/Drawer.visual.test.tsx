...
describe('Drawer visual parity with @mui/joy', () => {
  for (const scheme of COLOR_SCHEMES) {
    for (const variant of VARIANTS) {
      for (const color of COLORS) {
        it(`${variant}/${color} matches Joy UI's computed styles in ${scheme}`, async () => {
          ...
          await expect(page.getByTestId(`joy-${variant}-${color}`)).toMatchScreenshot(`drawer-${variant}-${color}-joy-${scheme}`);
          await expect(page.getByTestId(`hintoric-${variant}-${color}`)).toMatchScreenshot(`drawer-${variant}-${color}-hintoric-${scheme}`);
        });
      }
    }

    for (const anchor of ['left', 'right', 'top', 'bottom'] as const) {
      it(`anchor=${anchor} matches Joy UI's computed dimensions in ${scheme}`, async () => {
        await setColorScheme(scheme);
    ...
    for (const size of ['sm', 'md', 'lg'] as const) {
      it(`size=${size} matches Joy UI's computed width (horizontal anchor) in ${scheme}`, async () => {
        await setColorScheme(scheme);
