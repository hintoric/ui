    ...
    for (const variant of VARIANTS) {
      for (const color of COLORS) {
        it(`${variant}/${color} matches Joy UI's computed styles in ${scheme}`, async () => {
          ...
          await expect(joyLocator).toMatchScreenshot(`sheet-${variant}-${color}-joy-${scheme}`);
          await expect(hintoricLocator).toMatchScreenshot(`sheet-${variant}-${color}-hintoric-${scheme}`);
        });
      }
    }
  }
