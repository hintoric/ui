  ...
  for (const scheme of COLOR_SCHEMES) {
    for (const variant of VARIANTS) {
      for (const color of COLORS) {
        it(`${variant}/${color} matches Joy UI's computed item background (Accordion's own root) in ${scheme}`, async () => {
          ...
          await expect(page.getByTestId(`joy-${variant}-${color}`)).toMatchScreenshot(`accordion-${variant}-${color}-joy-${scheme}`);
          await expect(page.getByTestId(`hintoric-${variant}-${color}`)).toMatchScreenshot(`accordion-${variant}-${color}-hintoric-${scheme}`);
        });
      }
    }
    ...
    it(`AccordionSummary stays transparent regardless of the Accordion's own variant/color in ${scheme}`, async () => {
      await setColorScheme(scheme);
    ...
    it(`AccordionDetails applies its own variant/color when explicitly given in ${scheme}`, async () => {
      await setColorScheme(scheme);
    ...
    it(`expanding toggles aria-expanded and reveals the details panel, matching Joy UI in ${scheme}`, async () => {
      await setColorScheme(scheme);
