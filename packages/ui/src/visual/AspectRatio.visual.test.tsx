...
describe('AspectRatio visual parity with @mui/joy', () => {
  for (const scheme of COLOR_SCHEMES) {
    ...
    for (const [ratio, expected] of RATIOS) {
      it(`ratio=${ratio} achieves the same aspect ratio as Joy UI in ${scheme}`, async () => {
        ...
        await expect(page.getByTestId(`joy-${ratio}`)).toMatchScreenshot(`aspectratio-${ratio.replace('/', '-')}-joy-${scheme}`);
        await expect(page.getByTestId(`hintoric-${ratio}`)).toMatchScreenshot(`aspectratio-${ratio.replace('/', '-')}-hintoric-${scheme}`);
      });
    }
  }
});
