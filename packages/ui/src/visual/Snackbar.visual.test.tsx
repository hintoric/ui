...
describe('Snackbar visual parity with @mui/joy', () => {
  for (const scheme of COLOR_SCHEMES) {
    for (const variant of VARIANTS) {
      for (const color of COLORS) {
        it(`${variant}/${color} matches Joy UI's computed styles in ${scheme}`, async () => {
          ...
          await expect(page.getByTestId(`joy-${variant}-${color}`)).toMatchScreenshot(`snackbar-${variant}-${color}-joy-${scheme}`);
          await expect(page.getByTestId(`hintoric-${variant}-${color}`)).toMatchScreenshot(`snackbar-${variant}-${color}-hintoric-${scheme}`);
        });
      }
    }

    for (const size of ['sm', 'md', 'lg'] as const) {
      it(`size=${size} matches Joy UI's computed padding and min-width in ${scheme}`, async () => {
        await setColorScheme(scheme);
    ...
    for (const anchorOrigin of ANCHORS) {
      it(`anchorOrigin=${anchorOrigin.vertical}/${anchorOrigin.horizontal} matches Joy UI's computed position in ${scheme}`, async () => {
        await setColorScheme(scheme);
