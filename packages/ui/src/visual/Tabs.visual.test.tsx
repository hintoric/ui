  ...
  for (const scheme of COLOR_SCHEMES) {
    for (const variant of VARIANTS) {
      for (const color of COLORS) {
        it(`Tabs root ${variant}/${color} matches Joy UI's computed styles in ${scheme}`, async () => {
    ...
    for (const size of ['sm', 'md', 'lg'] as const) {
      it(`size=${size} matches Joy UI's computed Tab height and TabPanel padding in ${scheme}`, async () => {
        await setColorScheme(scheme);
