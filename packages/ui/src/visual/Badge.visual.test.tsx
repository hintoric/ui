    ...
    for (const variant of VARIANTS) {
      for (const color of COLORS) {
        it(`${variant}/${color} matches Joy UI's computed styles in ${scheme}`, async () => {
          ...
          await expect(page.getByTestId(`joy-${variant}-${color}`)).toMatchScreenshot(`badge-${variant}-${color}-joy-${scheme}`);
          await expect(page.getByTestId(`hintoric-${variant}-${color}`)).toMatchScreenshot(`badge-${variant}-${color}-hintoric-${scheme}`);
        });
      }
    }

    it(`hides the badge via invisible, matching Joy UI in ${scheme}`, async () => {
      await setColorScheme(scheme);
