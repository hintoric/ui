...
describe('AvatarGroup visual parity with @mui/joy', () => {
  for (const scheme of COLOR_SCHEMES) {
    for (const size of ['sm', 'md', 'lg'] as const) {
      it(`size=${size} overlap matches Joy UI in ${scheme}`, async () => {
        ...
        await expect(page.getByTestId(`joy-${size}`)).toMatchScreenshot(`avatargroup-${size}-joy-${scheme}`);
        await expect(page.getByTestId(`hintoric-${size}`)).toMatchScreenshot(`avatargroup-${size}-hintoric-${scheme}`);
      });
    }
  }
});
