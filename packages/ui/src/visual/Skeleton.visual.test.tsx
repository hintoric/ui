...
describe('Skeleton visual parity with @mui/joy', () => {
  for (const scheme of COLOR_SCHEMES) {
    ...
    for (const scheme of ['light', 'dark'] as const) {
      it(`matches Joy UI's computed background color in ${scheme} mode in ${scheme}`, async () => {
        await setColorScheme(scheme);
        ...
        await expect(page.getByTestId(`joy-${scheme}`)).toMatchScreenshot(`skeleton-${scheme}-joy-${scheme}`);
        await expect(page.getByTestId(`hintoric-${scheme}`)).toMatchScreenshot(`skeleton-${scheme}-hintoric-${scheme}`);
      });
    }
  }
});
