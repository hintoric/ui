  ...
  for (const scheme of COLOR_SCHEMES) {
    for (const variant of VARIANTS) {
      for (const color of COLORS) {
        it(`${variant}/${color} matches Joy UI's computed track/thumb styles in ${scheme}`, async () => {
          ...
          await expect(page.getByTestId(`joy-root-${variant}-${color}`)).toMatchScreenshot(`slider-${variant}-${color}-joy-${scheme}`);
          await expect(hintoricControl).toMatchScreenshot(`slider-${variant}-${color}-hintoric-${scheme}`);
        });
      }
    }

    for (const size of ['sm', 'md', 'lg'] as const) {
      it(`size=${size} matches Joy UI's computed thumb dimensions in ${scheme}`, async () => {
        await setColorScheme(scheme);
    ...
    it(`disabled slider matches Joy UI (dimmed, non-interactive) in ${scheme}`, async () => {
      await setColorScheme(scheme);
