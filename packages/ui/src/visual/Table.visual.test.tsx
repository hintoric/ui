...
describe('Table visual parity with @mui/joy', () => {
  for (const scheme of COLOR_SCHEMES) {
    for (const variant of VARIANTS) {
      for (const color of COLORS) {
        it(`${variant}/${color} matches Joy UI's computed styles in ${scheme}`, async () => {
          ...
          await expect(page.getByTestId(`joy-${variant}-${color}`)).toMatchScreenshot(`table-${variant}-${color}-joy-${scheme}`);
          await expect(page.getByTestId(`hintoric-${variant}-${color}`)).toMatchScreenshot(`table-${variant}-${color}-hintoric-${scheme}`);
        });
      }
    }

    for (const size of ['sm', 'md', 'lg'] as const) {
      it(`size=${size} matches Joy UI's computed cell height/padding in ${scheme}`, async () => {
        await setColorScheme(scheme);
    ...
    for (const borderAxis of AXES) {
      it(`borderAxis=${borderAxis} matches Joy UI's computed cell border widths in ${scheme}`, async () => {
        await setColorScheme(scheme);
    ...
    it(`hoverRow wires up a hover background class matching the hover token in ${scheme}`, async () => {
      await setColorScheme(scheme);
