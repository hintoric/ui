...
describe('Select visual parity with @mui/joy', () => {
  for (const scheme of COLOR_SCHEMES) {
    for (const variant of VARIANTS) {
      for (const color of COLORS) {
        it(`${variant}/${color} matches Joy UI's computed styles in ${scheme}`, async () => {
          ...
          await expect(page.getByTestId(`joy-${variant}-${color}`)).toMatchScreenshot(`select-${variant}-${color}-joy-${scheme}`);
          await expect(page.getByTestId(`hintoric-${variant}-${color}`)).toMatchScreenshot(`select-${variant}-${color}-hintoric-${scheme}`);
        });
      }
    }

    for (const size of ['sm', 'md', 'lg'] as const) {
      it(`size=${size} matches Joy UI's computed metrics in ${scheme}`, async () => {
        await setColorScheme(scheme);
        ...
        await expect(page.getByTestId(`joy-size-${size}`)).toMatchScreenshot(`select-size-${size}-joy-${scheme}`);
        await expect(page.getByTestId(`hintoric-size-${size}`)).toMatchScreenshot(`select-size-${size}-hintoric-${scheme}`);
      });
    }

    it(`plain variant has no box-shadow (unlike every other variant) in ${scheme}`, async () => {
      await setColorScheme(scheme);
    ...
    for (const color of COLORS) {
      it(`focus ring for color=${color} matches Joy UI's ring in ${scheme}`, async () => {
        await setColorScheme(scheme);
        ...
        await expect(page.getByTestId(`hintoric-focus-${color}`)).toMatchScreenshot(`select-focus-${color}-hintoric-${scheme}`);
        hintoricButton.blur();
    ...
    for (const variant of VARIANTS) {
      it(`disabled ${variant} matches Joy UI's computed styles in ${scheme}`, async () => {
        await setColorScheme(scheme);
        ...
        await expect(page.getByTestId(`joy-disabled-${variant}`)).toMatchScreenshot(`select-disabled-${variant}-joy-${scheme}`);
        await expect(page.getByTestId(`hintoric-disabled-${variant}`)).toMatchScreenshot(`select-disabled-${variant}-hintoric-${scheme}`);
      });
    }

    for (const variant of VARIANTS) {
      it(`hover ${variant} matches Joy UI's computed background in ${scheme}`, async () => {
        await setColorScheme(scheme);
    ...
    it(`open listbox surface matches Joy UI's computed styles in ${scheme}`, async () => {
      await setColorScheme(scheme);
