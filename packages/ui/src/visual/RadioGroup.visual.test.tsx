...
describe('RadioGroup error state', () => {
  for (const scheme of COLOR_SCHEMES) {
    it(`cascades danger into its Radios and helper text like Joy UI in ${scheme}`, async () => {
      await setColorScheme(scheme);
      ...
      await expect(page.getByTestId('joy-rg-error')).toMatchScreenshot(`radiogroup-error-joy-${scheme}`);
      await expect(page.getByTestId('hintoric-rg-error')).toMatchScreenshot(`radiogroup-error-hintoric-${scheme}`);
    });
  }
});
