...
describe('Link visual parity with @mui/joy', () => {
  for (const scheme of COLOR_SCHEMES) {
    for (const color of COLORS) {
      it(`plain (no variant) color=${color} matches Joy UI's computed styles in ${scheme}`, async () => {
        ...
        await expect(page.getByTestId(`joy-${color}`)).toMatchScreenshot(`link-plain-${color}-joy-${scheme}`);
        await expect(page.getByTestId(`hintoric-${color}`)).toMatchScreenshot(`link-plain-${color}-hintoric-${scheme}`);
      });
    }

    for (const variant of VARIANTS) {
      it(`variant=${variant} matches Joy UI's computed styles in ${scheme}`, async () => {
        await setColorScheme(scheme);
    ...
    it(`underline="always" applies text-decoration:underline, matching Joy UI in ${scheme}`, async () => {
      await setColorScheme(scheme);
