...
describe('FormField renders the same shell as a bound field', () => {
  it('matches the hand-composed Joy stack with helper text', async () => {
    ...
    await expect(page.getByTestId('joy-formfield')).toMatchScreenshot('formfield-helper-joy');
    await expect(page.getByTestId('hintoric-formfield')).toMatchScreenshot(
      'formfield-helper-hintoric',
    );
  });
});
